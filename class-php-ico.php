...
/**
 * A PHP class that uses the GD library to convert an image to an .ico file
 * with the correct mime type: image/x-icon
 *
...
class PHP_ICO {
	...
	public function __construct($file = false, $sizes = array()) {
		...
		foreach($required_functions as $function) {
			if(!function_exists($function)) {
				trigger_error("The PHP_ICO class was unable to find the " . $function . " function, which is part of the
				GD library. Ensure that the system has the GD library installed and that PHP has access to it through a PHP interface, such as PHP's GD module. Since this function was not found, the library will be unable to create ICO files.");
				return;
			}
		}
	...
	private function _load_image_file($file) {
		// Run a cheap check to verify that $file it is an image file.
